...
CREATE TABLE Schedule (
    ...
    bus_id         INT          NOT NULL REFERENCES Bus(bus_id) ON DELETE SET NULL,
    route_id       INT          NOT NULL REFERENCES Route(route_id) ON DELETE SET NULL,
...
-- Users with their assigned roles (created this part for backend)
SELECT
    u.user_id,
    u.first_name || ' ' || u.surname  AS full_name,
    u.email,
...
-- -------------------------------------------------------------------
-- 7K: BUSINESS INFORMATION QUERIES (it is 10 marks on the rubric, please check if it deserves full marks)
